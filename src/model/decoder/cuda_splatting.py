...
def _make_rasterization_settings(**kwargs) -> GaussianRasterizationSettings:
    if "debug" in getattr(GaussianRasterizationSettings, "_fields", ()):
        kwargs["debug"] = False
    return GaussianRasterizationSettings(**kwargs)


def _unpack_render_output(output):
    if len(output) == 4:
        image, radii, depth, _ = output
    elif len(output) == 3:
        image, radii, depth = output
    else:
        raise RuntimeError(
            f"Unexpected rasterizer output length: {len(output)}"
        )
    return image, radii, depth


def _unpack_image_output(output):
    if len(output) == 4:
        image, radii, _, _ = output
    elif len(output) == 3:
        image, radii, _ = output
    elif len(output) == 2:
        image, radii = output
    else:
        raise RuntimeError(
...
def render_cuda(
    ...
    for i in range(b):
        ...
        settings = _make_rasterization_settings(
            image_height=h,
            image_width=w,
        ...
        image, radii, depth = _unpack_render_output(
            rasterizer(
...
def render_cuda_orthographic(
    ...
    for i in range(b):
        ...
        image, radii = _unpack_image_output(
            rasterizer(
                means3D=gaussian_means[i],
                means2D=mean_gradients,
                shs=shs[i] if use_sh else None,
